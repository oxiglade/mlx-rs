...
| --- | --- | --- | --- |
| foundation, tranche 4b item 3 | `src/model/hub.rs`, `src/model/hub/snapshot.rs` | Integrate the exact declarations in `src/model/hub/INTEGRATION.md`; run the four public model-loading tests on the host and the two frozen Hub case tests when their fixture arrives | `hub::load` resolves one full commit, validates exact assets and completion receipts before/after `from_dir`, and attaches provenance. Offline resolution forbids transport/client construction. Shared declarations remain outside this item's ownership. |
