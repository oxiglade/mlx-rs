...
fn prepare_mlx_c_source() -> PathBuf {
    ...
    let patched = cmake_content.replace(
        "GIT_TAG v0.30.6)",
        "GIT_TAG v0.32.0\n    PATCH_COMMAND git apply ${CMAKE_CURRENT_SOURCE_DIR}/patches/metallib-search-path.patch || true\n    COMMAND sh -c \"git apply --check '${CMAKE_CURRENT_SOURCE_DIR}/patches/expert-aligned-gather.patch' && git apply '${CMAKE_CURRENT_SOURCE_DIR}/patches/expert-aligned-gather.patch' || git apply --reverse --check '${CMAKE_CURRENT_SOURCE_DIR}/patches/expert-aligned-gather.patch'\")",
    );
