use half::{bf16, f16};
use mlx_rs::{ops, with_new_default_stream, Array, Device, Dtype, Stream, StreamOrDevice};
...
fn dtype_from_name(name: &str) -> Result<Dtype, String> {
    match name {
        "BOOL" => Ok(Dtype::Bool),
        "U8" => Ok(Dtype::Uint8),
        "U16" => Ok(Dtype::Uint16),
...
impl<'a> Args<'a> {
    fn new(case: &'a Case, safe: &'a SafeTensors<'a>) -> Self {
        Self {
            case,
            safe,
    ...
    fn take(&mut self, name: &str) -> Result<&'a Arg, String> {
        let index = self
            .case
            .args
            .iter()
    ...
    fn tensor(&mut self, name: &str) -> Result<Array, String> {
    ...
    fn scalar(&mut self, name: &str) -> Result<ScalarValue, String> {
        match self.take(name)? {
            Arg::Scalar {
                scalar_type,
                value,
    ...
    fn axes(&mut self, name: &str) -> Result<Vec<i32>, String> {
        match self.take(name)? {
    ...
    fn axis(&mut self, name: &str) -> Result<i32, String> {
        match self.take(name)? {
    ...
    fn optional_axis(&mut self, name: &str) -> Result<Option<i32>, String> {
        match self.take(name)? {
    ...
    fn shape(&mut self, name: &str) -> Result<Vec<i32>, String> {
        match self.take(name)? {
    ...
    fn execution(&mut self) -> Result<ExecutionTarget, String> {
...
fn mlx_error<T>(result: mlx_rs::error::Result<T>) -> Result<T, String> {
    result.map_err(|error| error.to_string())
}

fn dispatch(case: &Case, safe: &SafeTensors<'_>) -> Result<Vec<Array>, String> {
